-- -------------------------------------------------------------
-- 
-- File Name: hdl_32bits\hdlsrc\fec_ber_hw_V2\decoder_control_dut.vhd
-- Created: 2022-10-27 15:30:21
-- 
-- Generated by MATLAB 9.9 and HDL Coder 3.17
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: decoder_control_dut
-- Source Path: decoder_control/decoder_control_dut
-- Hierarchy Level: 1
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;

ENTITY decoder_control_dut IS
  PORT( clk                               :   IN    std_logic;
        reset                             :   IN    std_logic;
        dut_enable                        :   IN    std_logic;  -- ufix1
        aximm_ctrl_word                   :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
        aximm_length                      :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
        aximm_load                        :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
        s00_axis_tvalid                   :   IN    std_logic;  -- ufix1
        s00_axis_tdata                    :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
        s01_axis_tvalid                   :   IN    std_logic;  -- ufix1
        s01_axis_tdata                    :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
        s02_axis_tvalid                   :   IN    std_logic;  -- ufix1
        s02_axis_tdata                    :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
        s03_axis_tvalid                   :   IN    std_logic;  -- ufix1
        s03_axis_tdata                    :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
        s_axis_status_tvalid              :   IN    std_logic;  -- ufix1
        s_axis_status_tdata               :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
        ce_out                            :   OUT   std_logic;  -- ufix1
        m_axis_ctrl_tvalid                :   OUT   std_logic;  -- ufix1
        m_axis_ctrl_tdata                 :   OUT   std_logic_vector(31 DOWNTO 0);  -- ufix32
        m_axis_ctrl_tlast                 :   OUT   std_logic;  -- ufix1
        m00_axis_tvalid                   :   OUT   std_logic;  -- ufix1
        m00_axis_tdata                    :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
        m01_axis_tvalid                   :   OUT   std_logic;  -- ufix1
        m01_axis_tdata                    :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
        m02_axis_tvalid                   :   OUT   std_logic;  -- ufix1
        m02_axis_tdata                    :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
        m03_axis_tvalid                   :   OUT   std_logic;  -- ufix1
        m03_axis_tdata                    :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
        s_axis_status_tready              :   OUT   std_logic  -- ufix1
        );
END decoder_control_dut;


ARCHITECTURE rtl OF decoder_control_dut IS

  -- Component Declarations
  COMPONENT decoder_control_src_FEC_Ctrl
    PORT( clk                             :   IN    std_logic;
          clk_enable                      :   IN    std_logic;
          reset                           :   IN    std_logic;
          aximm_ctrl_word                 :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
          aximm_length                    :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
          aximm_load                      :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
          s00_axis_tvalid                 :   IN    std_logic;  -- ufix1
          s00_axis_tdata                  :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
          s01_axis_tvalid                 :   IN    std_logic;  -- ufix1
          s01_axis_tdata                  :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
          s02_axis_tvalid                 :   IN    std_logic;  -- ufix1
          s02_axis_tdata                  :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
          s03_axis_tvalid                 :   IN    std_logic;  -- ufix1
          s03_axis_tdata                  :   IN    std_logic_vector(127 DOWNTO 0);  -- ufix128
          s_axis_status_tvalid            :   IN    std_logic;  -- ufix1
          s_axis_status_tdata             :   IN    std_logic_vector(31 DOWNTO 0);  -- ufix32
          ce_out                          :   OUT   std_logic;  -- ufix1
          m_axis_ctrl_tvalid              :   OUT   std_logic;  -- ufix1
          m_axis_ctrl_tdata               :   OUT   std_logic_vector(31 DOWNTO 0);  -- ufix32
          m_axis_ctrl_tlast               :   OUT   std_logic;  -- ufix1
          m00_axis_tvalid                 :   OUT   std_logic;  -- ufix1
          m00_axis_tdata                  :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
          m01_axis_tvalid                 :   OUT   std_logic;  -- ufix1
          m01_axis_tdata                  :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
          m02_axis_tvalid                 :   OUT   std_logic;  -- ufix1
          m02_axis_tdata                  :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
          m03_axis_tvalid                 :   OUT   std_logic;  -- ufix1
          m03_axis_tdata                  :   OUT   std_logic_vector(127 DOWNTO 0);  -- ufix128
          s_axis_status_tready            :   OUT   std_logic  -- ufix1
          );
  END COMPONENT;

  -- Component Configuration Statements
  FOR ALL : decoder_control_src_FEC_Ctrl
    USE ENTITY work.decoder_control_src_FEC_Ctrl(rtl);

  -- Signals
  SIGNAL enb                              : std_logic;
  SIGNAL s00_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL s01_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL s02_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL s03_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL s_axis_status_tvalid_sig         : std_logic;  -- ufix1
  SIGNAL ce_out_sig                       : std_logic;  -- ufix1
  SIGNAL m_axis_ctrl_tvalid_sig           : std_logic;  -- ufix1
  SIGNAL m_axis_ctrl_tdata_sig            : std_logic_vector(31 DOWNTO 0);  -- ufix32
  SIGNAL m_axis_ctrl_tlast_sig            : std_logic;  -- ufix1
  SIGNAL m00_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL m00_axis_tdata_sig               : std_logic_vector(127 DOWNTO 0);  -- ufix128
  SIGNAL m01_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL m01_axis_tdata_sig               : std_logic_vector(127 DOWNTO 0);  -- ufix128
  SIGNAL m02_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL m02_axis_tdata_sig               : std_logic_vector(127 DOWNTO 0);  -- ufix128
  SIGNAL m03_axis_tvalid_sig              : std_logic;  -- ufix1
  SIGNAL m03_axis_tdata_sig               : std_logic_vector(127 DOWNTO 0);  -- ufix128
  SIGNAL s_axis_status_tready_sig         : std_logic;  -- ufix1

BEGIN
  u_decoder_control_src_FEC_Ctrl : decoder_control_src_FEC_Ctrl
    PORT MAP( clk => clk,
              clk_enable => enb,
              reset => reset,
              aximm_ctrl_word => aximm_ctrl_word,  -- ufix32
              aximm_length => aximm_length,  -- ufix32
              aximm_load => aximm_load,  -- ufix32
              s00_axis_tvalid => s00_axis_tvalid_sig,  -- ufix1
              s00_axis_tdata => s00_axis_tdata,  -- ufix128
              s01_axis_tvalid => s01_axis_tvalid_sig,  -- ufix1
              s01_axis_tdata => s01_axis_tdata,  -- ufix128
              s02_axis_tvalid => s02_axis_tvalid_sig,  -- ufix1
              s02_axis_tdata => s02_axis_tdata,  -- ufix128
              s03_axis_tvalid => s03_axis_tvalid_sig,  -- ufix1
              s03_axis_tdata => s03_axis_tdata,  -- ufix128
              s_axis_status_tvalid => s_axis_status_tvalid_sig,  -- ufix1
              s_axis_status_tdata => s_axis_status_tdata,  -- ufix32
              ce_out => ce_out_sig,  -- ufix1
              m_axis_ctrl_tvalid => m_axis_ctrl_tvalid_sig,  -- ufix1
              m_axis_ctrl_tdata => m_axis_ctrl_tdata_sig,  -- ufix32
              m_axis_ctrl_tlast => m_axis_ctrl_tlast_sig,  -- ufix1
              m00_axis_tvalid => m00_axis_tvalid_sig,  -- ufix1
              m00_axis_tdata => m00_axis_tdata_sig,  -- ufix128
              m01_axis_tvalid => m01_axis_tvalid_sig,  -- ufix1
              m01_axis_tdata => m01_axis_tdata_sig,  -- ufix128
              m02_axis_tvalid => m02_axis_tvalid_sig,  -- ufix1
              m02_axis_tdata => m02_axis_tdata_sig,  -- ufix128
              m03_axis_tvalid => m03_axis_tvalid_sig,  -- ufix1
              m03_axis_tdata => m03_axis_tdata_sig,  -- ufix128
              s_axis_status_tready => s_axis_status_tready_sig  -- ufix1
              );

  s00_axis_tvalid_sig <= s00_axis_tvalid;

  s01_axis_tvalid_sig <= s01_axis_tvalid;

  s02_axis_tvalid_sig <= s02_axis_tvalid;

  s03_axis_tvalid_sig <= s03_axis_tvalid;

  s_axis_status_tvalid_sig <= s_axis_status_tvalid;

  enb <= dut_enable;

  ce_out <= ce_out_sig;

  m_axis_ctrl_tvalid <= m_axis_ctrl_tvalid_sig;

  m_axis_ctrl_tdata <= m_axis_ctrl_tdata_sig;

  m_axis_ctrl_tlast <= m_axis_ctrl_tlast_sig;

  m00_axis_tvalid <= m00_axis_tvalid_sig;

  m00_axis_tdata <= m00_axis_tdata_sig;

  m01_axis_tvalid <= m01_axis_tvalid_sig;

  m01_axis_tdata <= m01_axis_tdata_sig;

  m02_axis_tvalid <= m02_axis_tvalid_sig;

  m02_axis_tdata <= m02_axis_tdata_sig;

  m03_axis_tvalid <= m03_axis_tvalid_sig;

  m03_axis_tdata <= m03_axis_tdata_sig;

  s_axis_status_tready <= s_axis_status_tready_sig;

END rtl;

